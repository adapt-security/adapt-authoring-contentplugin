...
class ContentPluginModule extends AbstractApiModule {
  ...
  async getMissingPlugins () {
    ...
    const pluginsWithPaths = await Promise.all(missingPlugins.map(async (p) => {
      ...
      try {
        ...
        // Main directory doesn't exist, check for backups
        // Log unexpected errors (anything other than ENOENT/ENOTDIR)
        if (e.code && e.code !== 'ENOENT' && e.code !== 'ENOTDIR') {
          this.log('warn', `Unexpected error accessing ${pluginPath}: ${e.code}`)
        }

        const pattern = `${p.name}-v*`
        const backups = await glob(pattern, { cwd: pluginDir, absolute: true })

        if (backups.length > 0) {
          // Sort backups by modification time (newest first) and use the most recent
          // We use mtime because it's reliable and doesn't require parsing version strings
          // which may have different formats (semver, timestamps for unknown versions, etc.)
          ...
          return `${p.name}@${backupsWithStats[0].path}`
        }
  ...
  async backupPluginVersion (pluginPath, pluginName) {
    try {
      ...
      // Plugin doesn't exist, no backup needed
      return null
    }

    // Read the version from the existing plugin
    let existingVersion
    try {
  ...
  async cleanupOldPluginBackups (pluginDir, pluginName) {
    ...
    if (backups.length <= 1) {
      // Keep the most recent backup (or no backups exist)
      return
    }
    ...
    const backupsToRemove = backupsWithStats.slice(1)
    for (const backup of backupsToRemove) {
      await fs.rm(backup.path, { recursive: true })
      this.log('info', `Removed old backup: ${backup.path}`)
    }
  }
  ...
  async restorePluginFromBackup (pluginName) {
    ...
    backupsWithStats.sort((a, b) => b.mtime - a.mtime)
    const mostRecentBackup = backupsWithStats[0].path
    ...
    // Read and return the restored plugin info
    let pkg
    try {
